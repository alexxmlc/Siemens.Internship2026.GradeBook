Violation 1:
    ...
    2. Where in the code: In Program.cs it was missing dependency injection, in ItemController it had a 
        constructor injection failure 

    3. Why is it a violation: Even though the ItemController correctly depends on the interface not on the 
    ...
    4. The fix: I registered the required dependency in Program.cs by adding: 
        builder.Services.AddScoped<IItemReader, ItemRepository>();. This ensures a loose coupling so when the 
...
Violation 2:
    ...
    2. Where in the code: In ItemController.cs in functions GetAll() and GetById()

    3. Why is it a violation: Controllers have one job, to handle HTTP requests and returning HTTP responses,
        however these methods mix business logic/logging with routing (calculating the average:  itemList.Average(i => i.Value) : 0)
